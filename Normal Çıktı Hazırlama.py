...
def create_bat_files(data, output_folder, batch_size=28):
    ...
    while len(data) > 0:
        ...
        with open(bat_file_path, "w") as file:
            ...
            first_link = f'start "" https://task.haydigiy.com/admin/order/printorder/?orderId={current_batch[0]}&isPdf=False\n'
            file.write(first_link)
            file.write('timeout -t 2\n')  # 2 saniye bekleme süresi

            # Kalan linkleri ekle
            for order_id in current_batch[1:]:
                link = f'start "" https://task.haydigiy.com/admin/order/printorder/?orderId={order_id}&isPdf=False\n'
                file.write(link)
...
for file_name in excel_files:
    ...
    # 2. "Id" sütunundaki verileri al
    id_values = [
        cell.value for cell in unique_ids_sheet["A"] if cell.value is not None
    ]

    # 3. İşlenen dosya için klasör oluştur
    output_folder = os.path.splitext(file_name)[0]  # Dosya adını al (uzantısız)
    os.makedirs(output_folder, exist_ok=True)

    # 4. BAT dosyalarını oluştur ve klasöre kaydet
    create_bat_files(id_values, output_folder)

    # 5. İşlenmiş Excel dosyasını klasöre taşı
    processed_excel_path = os.path.join(output_folder, file_name)
    wb.save(processed_excel_path)  # Dosyanın son hali klasöre kaydedilir

    # 6. Klasör dışındaki orijinal Excel dosyasını sil
    gc.collect()
    if os.path.exists(file_name):
        os.remove(file_name)
